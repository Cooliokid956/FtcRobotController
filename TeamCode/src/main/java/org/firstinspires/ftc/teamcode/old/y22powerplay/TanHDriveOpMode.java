...
public class TanHDriveOpMode extends OpMode {
    ...
    public void init() {
        ...
        lift.lift = hardwareMap.get(DcMotor.class,"liftMotor");
...
//            robot.cMotor.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
        ...
        robot.lMotor.setPower(((forwardPower + turn) / max * magnitude)*bti(modeMove)+(-gamepad1.left_stick_y)*bti(!modeMove));
        robot.rMotor.setPower(((forwardPower - turn) / max * magnitude)*bti(modeMove)+(-gamepad1.right_stick_y)*bti(!modeMove));
